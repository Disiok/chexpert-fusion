...
class VolumetricFusionDenseNet(nn.Module):
    def __init__(self, num_classes=10, num_init_features=64, normalization=None, activation=None, is_big=False, use_2d_conv=False):
        ...
        if use_2d_conv:
            self.input_fusion = fusions.VolumetricFusion2D(num_init_features)
        else:
            self.input_fusion = fusions.VolumetricFusion(num_init_features, is_big=is_big)
...
def make_volumetric_densenet121(config):
    ...
    model = VolumetricFusionDenseNet(
        ...
        is_big=config['model']['is_big'],
        use_2d_conv=config['model']['use_2d_conv'],
    )
    return model
